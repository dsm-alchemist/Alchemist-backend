...
public class TaskController {
    ...
    @GetMapping
    public ResponseEntity<TaskListResponse> getTaskList(@RequestParam LocalDate date) {
        return taskService.getMyTaskList(date);
    }

    @PutMapping("/{taskId}")
    @ResponseStatus(HttpStatus.CREATED)
    public void modifyTask(@PathVariable("taskId") Long task_id, @RequestBody String task) {
        taskService.modifyTask(task_id, task);
    }

    @PostMapping("/{taskId}")
    @ResponseStatus(HttpStatus.CREATED)
    public void modifyDate(@PathVariable("taskId") Long task_id, @RequestParam LocalDate date) {
